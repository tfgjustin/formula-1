...
def head_to_head(h2h_predictions):
    ...
    for parts in h2h_predictions:
        confidence = float(parts[_CONFIDENCE_IDX])
        if confidence <= 0.5:
            continue
        correct = int(parts[_CORRECT_IDX])
        diff = correct - confidence
        if parts[_EVENT_IDX].endswith('Q'):
            qual_num += 1
            qual_correct += correct
            qual_exp += confidence
            qual_sse += (diff * diff)
        elif parts[_EVENT_IDX].endswith('R'):
            race_num += 1
    ...
    if race_num:
        print('Races\t%7d\t%7d\t%.4f\t%.4f\t%.4f' % (race_num, race_correct,
            ...
            race_sse / race_num))
    if qual_num:
        print('Quali\t%7d\t%7d\t%.4f\t%.4f\t%.4f' % (qual_num, qual_correct,
